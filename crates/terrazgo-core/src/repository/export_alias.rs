...
//!
//! In CORE rather than in an export crate or a module: the mechanism is
//! generic over `(target, entity_table, entity_id, split_key)` and aliases
//! registers owned by core and by all three modules. Minting them is a WRITE
//! to synced user data, which is core's job; deciding what to alias, and what
//! a `split_key` means, belongs to whichever serializer is exporting.
//!
...
pub fn ensure_export_alias(
    ...
    let row = ExportAlias {
        ...
        alias: next,
        created_at: crate::date::now_utc_iso(),
    };
    tx.execute(
