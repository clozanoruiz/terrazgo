...
// folded — so the RULES match. Which language's rules apply does not always:
// the book sorts in its own report language and this sorts in the one being
// read, which coincide whenever the app is read in the language the book
// prints in. Castilian files `ñ` after `n` (Peña after Penz) where Catalan and
// English fold it beside `n`, so an English reader of a Castilian book sees a
// different order on screen than on the printout — the same rows, differently
// filed.
//
// Framework-agnostic tier: no Svelte import (docs/frontend-conventions.md).
// languageTag, not the region format: names sort by the language being READ,
// never by the machine's regional settings, which have no opinion about words.
...
/// A query split into folded tokens, ready for `matchRank`.
///
/// Exported because the settings search matches the same way over a different
/// corpus (i18n strings rather than catalogue rows) and must not grow a second
/// idea of what "matches" means — see settingsTree.js.
export function foldTokens(query) {
...
export function matchRank(folded, tokens) {
  let best = 3;
...
export function searchItems(items, query, cap) {
  ...
  for (const item of items) {
    ...
    const score = matchRank(item.folded ?? fold(item.label), tokens);
